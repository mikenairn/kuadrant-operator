...
func (r *EffectiveDNSPoliciesReconciler) reconcile(ctx context.Context, _ []controller.ResourceEvent, topology *machinery.Topology, _ error, state *sync.Map) error {
	...
	for _, policy := range policies {
		...
		listeners := r.listenersForPolicy(ctx, topology, policy)
...
// If the target is a Gateway all listeners that do not have a policy explicitly attached are returned.
func (r *EffectiveDNSPoliciesReconciler) listenersForPolicy(_ context.Context, topology *machinery.Topology, policy machinery.Policy) []*machinery.Listener {

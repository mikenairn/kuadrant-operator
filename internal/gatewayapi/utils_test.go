...
func TestGetRouteAcceptedParentRefs_WithSectionName(t *testing.T) {
	...
	got := GetRouteAcceptedParentRefs(route)

	// Should return only the accepted parent ref (http section)
	if len(got) != 1 {
		t.Errorf("GetRouteAcceptedParentRefs() returned %d parent refs, want 1", len(got))
	}

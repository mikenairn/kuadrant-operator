...
sync-component-charts: ## Sync component Helm charts from upstream repos.
	go run $(PROJECT_PATH)/hack/sync-components/ --config $(COMPONENT_CHARTS_DIR)/sync.yaml
...
sync-component-chart: ## Sync a single component chart. Usage: make sync-component-chart COMPONENT=dns-operator
	go run $(PROJECT_PATH)/hack/sync-components/ --config $(COMPONENT_CHARTS_DIR)/sync.yaml --component $(COMPONENT)
